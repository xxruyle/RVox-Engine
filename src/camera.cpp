...
{
    for (float castLength = 0.1f; castLength < 4.0f; castLength+=0.1f) 
    {
        ...
        for (unsigned int i = 0; i < mWorld->positions.size(); i++) 
        {
            if (mWorld->positions[i] == origin) // if there is a block in the world that matches the "collides"  with the raycast vector 
            {  
                return origin;  
            }
        }
...
{
    glm::vec3 voxel_position = rayCast(); 
    for (unsigned int i = 0; i < mWorld->positions.size(); i++) 
    {
        if (mWorld->positions[i] == voxel_position)
        {
            mWorld->positions.erase(mWorld->positions.begin() + i); 
            break; 
        }
    }
}
